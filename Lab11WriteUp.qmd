...
ggplot(raw) +
  geom_boxplot(aes(x = Estimator,
                   y= Estimate),
               width = 0.1) +
  ggtitle("Distribution of S1 vs S2 across 1000 simulations") +
...
for(n in ss){
  ...
  S1_pois_n[[as.character(n)]] <- S1
  S2_pois_n[[as.character(n)]] <- S2
}
...
raw_pois <- tibble(
  Estimator= rep(c("S1", "S2"), each = 1000, times = 4),
  n=rep(ss, each = 2000),
  Estimate= c(S1_pois_n[["20"]],  S2_pois_n[["20"]],
              S1_pois_n[["50"]],  S2_pois_n[["50"]],
              S1_pois_n[["100"]], S2_pois_n[["100"]],
              S1_pois_n[["500"]], S2_pois_n[["500"]])
)
# summary table
results_pois <- tibble(
  Samplesize = rep(c(20, 50, 100, 500), each=2),
  Estimator = rep(c("S1", "S2"), times = 4),
  Bias      = c(
    mean(S1_pois_n[["20"]])  - 2, mean(S2_pois_n[["20"]])  - 2,
    mean(S1_pois_n[["50"]])  - 2, mean(S2_pois_n[["50"]])  - 2,
    mean(S1_pois_n[["100"]]) - 2, mean(S2_pois_n[["100"]]) - 2,
    mean(S1_pois_n[["500"]]) - 2, mean(S2_pois_n[["500"]]) - 2
  ),
  Variance  = c(
    var(S1_pois_n[["20"]]),  var(S2_pois_n[["20"]]),
    var(S1_pois_n[["50"]]),  var(S2_pois_n[["50"]]),
    var(S1_pois_n[["100"]]), var(S2_pois_n[["100"]]),
    var(S1_pois_n[["500"]]), var(S2_pois_n[["500"]])
  ),
  Precision = 1 / Variance,
  MSE       = Bias^2 + Variance)
